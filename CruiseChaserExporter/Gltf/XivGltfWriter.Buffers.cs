...
public partial class XivGltfWriter {
    ...
    private unsafe int AddBufferView<T>(
        ...
        int dataCount = -1)
        where T : unmanaged {
        dataCount = dataCount == -1 ? data.Length : dataCount;

        var rawSize = Marshal.SizeOf(data[0]) * dataCount;
    ...
    private int AddAccessor(string? baseName, int bufferView, GltfBufferViewTarget? bufferViewTarget, Vector2[] data,
        int start = 0, int end = int.MaxValue) {
        if (bufferView == -1)
            bufferView = AddBufferView(baseName, data, bufferViewTarget);
        if (end == int.MaxValue)
    ...
    private int AddAccessor(string? baseName, int bufferView, GltfBufferViewTarget? bufferViewTarget, Vector4[] data,
        int start = 0, int end = int.MaxValue) {
        if (bufferView == -1)
            bufferView = AddBufferView(baseName, data, bufferViewTarget);
        if (end == int.MaxValue)
    ...
    private int AddTexture<TPixel>(string? baseName, int width, int height, TPixel[] raw)
        ...
        using (var image = Image.LoadPixelData<TPixel>(raw, width, height))
            image.SaveAsPng(ms);
